// Paiza 問題集 クラス・構造体メニュー クラスのメンバの更新 C編（paizaランク C 相当）
// https://paiza.jp/works/mondai/class_primer/class_primer__change_member
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

struct employee {
    int number, i;
    char name[21];
    
    struct employee* pn;

    int (*getnumber)(struct employee* e);
    char* (*getname)(struct employee* e);
    void (*change_num)(struct employee* e, int num);
    void (*change_name)(struct employee* e, char* name);
};

struct employee* make(int a, char* name);
int getnumber(struct employee* e);
char* getname(struct employee* e);
void change_num(struct employee* e, int num);
void change_name(struct employee* e, char* name);

int main()
{
    int n;
    struct employee *ps = NULL, *pc = NULL;

    scanf("%d ", &n);
    for (int i = 0; i < n; i++) {
        char s[512];

        fflush(stdin);
        fgets(s, sizeof(s), stdin);
        s[strcspn(s, "\n")] = '\0';
        char* p = strtok(s, " ");
        if (strcmp(p, "make") == 0) {
            int a = atoi(strtok(NULL, " "));
            char* name = strtok(NULL, " ");
            struct employee** ppt = ps == NULL ? &ps : &pc->pn;

            *ppt = make(a, name);
            pc = *ppt;
        } else if (strcmp(p, "getnum") == 0) {
            int a = atoi(strtok(NULL, " "));
            struct employee* pb = ps;

            while (pb != NULL) {
                if (pb->i == a) {
                    printf("%d\n", pb->getnumber(pb));
                    break;
                }

                pb = pb->pn;
            }
        } else if (strcmp(p, "getname") == 0) {
            int a = atoi(strtok(NULL, " "));
            struct employee* pb = ps;

            while (pb != NULL) {
                if (pb->i == a) {
                    printf("%s\n", pb->getname(pb));
                    break;
                }

                pb = pb->pn;
            }
        } else if (strcmp(p, "change_num") == 0) {
            int a = atoi(strtok(NULL, " "));
            int num = atoi(strtok(NULL, " "));
            struct employee* pb = ps;

            while (pb != NULL) {
                if (pb->i == a) {
                    pb->change_num(pb, num);
                    break;
                }
                pb = pb->pn;
            }
        } else if (strcmp(p, "change_name") == 0) {
            int a = atoi(strtok(NULL, " "));
            char* name = strtok(NULL, " ");
            struct employee* pb = ps;

            while (pb != NULL) {
                if (pb->i == a) {
                    pb->change_name(pb, name);
                    break;
                }

                pb = pb->pn;
            }
        }
    }
    
    return 0;
}

struct employee* make(int a, char* name)
{
    static int i = 1;
    struct employee* p = (struct employee*)calloc(1, sizeof(struct employee));
    
    p->number = a;
    strcpy(p->name, name);
    p->getnumber = getnumber;
    p->getname = getname;
    p->change_num = change_num;
    p->change_name = change_name;
    p->i = i++;
    return p;
}

int getnumber(struct employee* e)
{
    return e->number;
}

char* getname(struct employee* e)
{
    return e->name;
}

void change_num(struct employee* e, int num)
{
    e->number = num;
}

void change_name(struct employee* e, char* name)
{
    strcpy(e->name, name);
}